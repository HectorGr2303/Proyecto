...
const createFormulario = async (req, res) => {
  try {
    const { usuarioId, nroFormulario } = req.body;
    const newFormulario = await formularioModel.createFormulario(nroFormulario);
...
const updateFormulario = async (req, res) => {
  try {
    const { id } = req.params;
    const { usuarioId, nroFormulario } = req.body;
    await formularioModel.updateFormulario(id, nroFormulario);
    res.sendStatus(204);
  } catch (error) {
    res.status(500).send(error.message);
  }
